...
species_sp_freq <- df_traits %>%
  ...
  mutate(
    species_names = paste(
      unique(
        df_traits$species[
          df_traits$Continent == Continent &   # match continent
...
ggplot(ground.cwm, aes(x = Years_since_fire, y = Long_cwm, 
                       by = Fire_Int_Groups))+
  geom_jitter(aes(color= Fire_Int_Groups))+
  geom_smooth(aes(color = Fire_Int_Groups))+
  facet_wrap(~Continent)

area_mod_lo <- glmmTMB(log(Long_cwm) ~ 
                         Fire_Int_Groups * Continent *
                         Years_since_fire +
                         Temp_sc +
                         (1 | StudyID),
...
area_mod_lo2 <- glmmTMB(log(Long_cwm) ~ 
                          Fire_Int_Groups * Continent *
                          Years_since_fire +
                          Temp_sc +
                          (1 | StudyID),
...
AIC_vals <- AIC(area_mod_lo, area_mod_lo2)
AIC_vals$delta <- AIC_vals$AIC - min(AIC_vals$AIC)
AIC_vals

summary(area_mod_lo)
Anova(area_mod_lo, type = 'III')
...
longevity_plot<- ggplot(pred_grid_lo,
                      aes(x = Years_since_fire,
                          y = fit,
                          color = Fire_Int_Groups)) +
  geom_ribbon(aes(ymin = lower, ymax = upper, fill = Fire_Int_Groups),
...
IndSizePlot <- ggplot(df_sizes, aes(x = Years_since_fire,
                                    y = log10(studysize))) +
...
#1. Remove all sp. species
richness_df <- df_long %>%
  filter(!grepl("_sp\\.$", species))

#2. Group dataset by unique species entries
richness_df2 <- richness_df %>%
  group_by(Fire_Int_Groups, Continent, PlantGroup) %>%
  summarize(
    Sp_tot = n_distinct(species),
    .groups = "drop"
  )

richness_df2$Fire_Int_Groups <- factor(
  richness_df2$Fire_Int_Groups,
  levels = c("Low", "Medium", "High")
)

richness_df2$PlantGroup <- factor(
  richness_df2$PlantGroup,
  levels = c("tree", "shrub", "herb", "dwarfshrub",
             "graminoid", "bryophyte")
)

richnessplot <- ggplot(richness_df2, 
                       aes(x = Fire_Int_Groups, y = Sp_tot)) +
  geom_col(aes(fill = Fire_Int_Groups)) +
  labs(
    x = "Years since fire",
    y = "Species richness",
  ...
  theme_bw() +
  facet_grid(Continent ~ PlantGroup,
             labeller = labeller(
               Continent = c(
                 "Eurasia" = "Eurasia",
                 "North_America" = "North America"),
...
#Diversity exploring
restricted_species <- richness_df %>%
  distinct(Continent, Fire_Int_Groups, species, PlantGroup) %>%   # remove duplicates
  group_by(Continent, species, PlantGroup) %>%
  filter(n_distinct(Fire_Int_Groups) == 1) %>%        # species occurs in only 1 fire intensity
  ungroup()

species_counts <- restricted_species %>%
  count(Continent, Fire_Int_Groups, PlantGroup, name = "n_unique_species")

species_counts$Fire_Int_Groups <- factor(
  species_counts$Fire_Int_Groups,
  levels = c("Low", "Medium", "High")
)

species_counts$PlantGroup <- factor(
  species_counts$PlantGroup,
  levels = c("tree", "shrub", "herb", "dwarfshrub",
             "graminoid", "bryophyte")
)

unique_plot <- ggplot(species_counts, aes (x = Fire_Int_Groups, y = n_unique_species)) +
  geom_col(aes(fill = Fire_Int_Groups)) +
  ylab("Unique species") +
  theme_bw() +
  facet_grid(Continent ~ PlantGroup,
...
species_sets <- richness_df %>%
  distinct(Continent, Fire_Int_Groups, species, PlantGroup)

species_sets <- species_sets %>%
  mutate(
    Fire_Int_Groups = factor(
      Fire_Int_Groups,
      levels = c("Low", "Medium", "High"),
      ordered = TRUE
    )
...
pairwise_overlap <- species_sets %>%
  inner_join(
    species_sets,
    by = c("Continent", "species", "PlantGroup"),
    suffix = c("_1", "_2")
  ) %>%
  filter(Fire_Int_Groups_1 < Fire_Int_Groups_2) %>%
  count(Continent, PlantGroup, Fire_Int_Groups_1, Fire_Int_Groups_2,
        name = "n_shared")

totals <- species_sets %>%
  count(Continent, Fire_Int_Groups, PlantGroup, name = "n_total")

pairwise_overlap <- pairwise_overlap %>%
  mutate(
    comparison = paste(Fire_Int_Groups_1, Fire_Int_Groups_2, sep = "-")
  )

result <- pairwise_overlap %>%
  left_join(totals, by = c("Continent", "PlantGroup", "Fire_Int_Groups_1" = "Fire_Int_Groups")) %>%
  rename(n1 = n_total) %>%
  left_join(totals, by = c("Continent", "PlantGroup", "Fire_Int_Groups_2" = "Fire_Int_Groups")) %>%
  rename(n2 = n_total) %>%
  mutate(
    proportion_shared = n_shared / (n1 + n2),
      jaccard = n_shared / (n1 + n2 - n_shared)
  )
...
result <- result %>%
  mutate(
    comparison = factor(
      comparison,
      levels = c("Low-Medium", "Medium-High", "Low-High"),
      ordered = TRUE
    )
  )

result$PlantGroup <- factor(
  result$PlantGroup,
  levels = c("tree", "shrub", "herb", "dwarfshrub",
             "graminoid", "bryophyte")
)
...
# Function to get midpoint color
mid_color <- function(col1, col2) {
  rgb(colorRamp(c(col1, col2))(0.5)/255)
}

# Compute midpoints
mid_corn_golden <- mid_color(cornflowerblue, goldenrod)
mid_golden_fire <- mid_color(goldenrod, firebrick)
mid_corn_fire   <- mid_color(cornflowerblue, firebrick)

mid_corn_golden
mid_golden_fire
mid_corn_fire

jaccardplot <- ggplot(result, aes(x = comparison, y = jaccard)) +
  ...
  scale_fill_manual(values=c("#9F9D87", "#C66421", "#8B5C88"))+
  theme(legend.position="right",
        legend.text=element_text(size=16),
        legend.title=element_text(size=18),
        legend.direction='vertical',
...
totals <- df_long %>%
  group_by(Fire_Int_Groups, Continent, PlantGroup) %>%
  summarise(total_rows = n_distinct(RowID), .groups = "drop")

richness_list <- df_long %>%
  group_by(Fire_Int_Groups, Continent, PlantGroup, species) %>%
  summarise(n_species = n(), .groups = "drop") %>%
  left_join(totals,
            by = c("Fire_Int_Groups", "Continent", "PlantGroup")) %>%
  mutate(frequency = n_species / total_rows) %>%
...
df_summary <- df_long %>%
  mutate(
    fire_bin = case_when(
      Years_since_fire >= 1 & Years_since_fire <= 5  ~ "1-5",
      Years_since_fire >= 6 & Years_since_fire <= 10 ~ "6-10",
      Years_since_fire > 10                          ~ "10+",
      TRUE ~ NA_character_
    )
  )
df_percent <- df_summary %>%
  filter(!is.na(fire_bin)) %>%
  group_by(Continent, fire_bin) %>%
  summarise(
    n_rowID   = n_distinct(RowID),
    n_studyID = n_distinct(StudyID),
    .groups = "drop"
  ) %>%
  ungroup()
range(df_long$studysize)
quantile(df_long$studysize, probs = c(0.05, 0.95), na.rm = TRUE)

df_long$StudyID <- as.numeric(df_long$StudyID)
range(df_long$StudyID)

df_long$RowID <- as.numeric(df_long$RowID)
range(df_long$RowID)
